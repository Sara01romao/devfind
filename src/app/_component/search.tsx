'use client';
import { userType } from "app/page";
import { SetStateAction, useState } from "react";

type searchProps = {
  searchUser: (value: SetStateAction<userType>) => void;
  setErroMsg: (value: SetStateAction<string>) => void;
}

export function Search({ searchUser, setErroMsg }: searchProps) {
  const [search, setSearch] = useState<string>('');
  const [isSearch, setIsSearch] = useState<boolean>(false);

  async function getUser() {
    setIsSearch(true)
    const response = await fetch(`https://api.github.com/users/${search}`);
    const data = await response.json();
   
    if (!response.ok) {
      setErroMsg("Não encontrado");
      searchUser(null);
      setIsSearch(false);
      return;
    }
    
    setIsSearch(false);
    searchUser(data);
  }

  function handleSearch(e: React.FormEvent) {
    e.preventDefault();

    if (!search) {
      searchUser(null);
      setErroMsg("Preencha o campo");
      return;
    }
    setErroMsg("");
    getUser();
  }

  return (
    <form onSubmit={handleSearch} className="max-w-[730px] w-full mx-auto flex bg-white dark:bg-[#1E2A47] p-2 px-2 sm:px-6 rounded-2xl shadow-lg shadow-gray-200/80 dark:shadow-none">
      <div className="w-full flex px-2 gap-2 items-center bg-white dark:bg-[#1E2A47]">
        <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path fillRule="evenodd" clipRule="evenodd" d="M10.6087 0C4.7591 0 0 4.74609 0 10.5797C0 16.4133 4.75915 21.1594 10.6087 21.1594C13.2162 21.1594 15.6071 20.2163 17.4563 18.6542L22.575 23.747C22.7449 23.9157 22.9664 24 23.1884 24C23.4118 24 23.635 23.9145 23.8049 23.7438C24.1435 23.4032 24.142 22.8527 23.8017 22.5139L18.6893 17.4274C20.2652 15.5807 21.2174 13.189 21.2174 10.5797C21.2174 4.74609 16.4582 0 10.6087 0ZM16.9346 16.7705C18.5071 15.1744 19.4782 12.9879 19.4782 10.5797C19.4782 5.70488 15.4994 1.73908 10.6087 1.73908C5.71794 1.73908 1.73913 5.70488 1.73913 10.5797C1.73913 15.4542 5.71794 19.4203 10.6087 19.4203C13.0163 19.4203 15.2029 18.4591 16.8027 16.9016C16.8211 16.879 16.8407 16.8571 16.8617 16.836C16.885 16.8125 16.9094 16.7907 16.9346 16.7705Z" fill="#0079FF" />
        </svg>

        <input type="text" onChange={(e) => setSearch(e.target.value)} placeholder="Search GitHub username…" className="rounded w-full h-[40px] p-2 dark:bg-[#1E2A47] dark:text-white" />
      </div>

      {isSearch ?
        <button type="submit" disabled className=" flex items-center text-base max-w-fit bg-[#026ee9] transition duration-500 font-bold text-white px-4 py-4 sm:px-6 rounded-[10px]">
          <svg className="mr-3 size-5 animate-spin ..." width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M14.8438 2.34375C14.8438 3.63818 13.7944 4.6875 12.5 4.6875C11.2056 4.6875 10.1562 3.63818 10.1562 2.34375C10.1562 1.04932 11.2056 0 12.5 0C13.7944 0 14.8438 1.04932 14.8438 2.34375ZM12.5 20.3125C11.2056 20.3125 10.1562 21.3618 10.1562 22.6562C10.1562 23.9507 11.2056 25 12.5 25C13.7944 25 14.8438 23.9507 14.8438 22.6562C14.8438 21.3618 13.7944 20.3125 12.5 20.3125ZM22.6562 10.1562C21.3618 10.1562 20.3125 11.2056 20.3125 12.5C20.3125 13.7944 21.3618 14.8438 22.6562 14.8438C23.9507 14.8438 25 13.7944 25 12.5C25 11.2056 23.9507 10.1562 22.6562 10.1562ZM4.6875 12.5C4.6875 11.2056 3.63818 10.1562 2.34375 10.1562C1.04932 10.1562 0 11.2056 0 12.5C0 13.7944 1.04932 14.8438 2.34375 14.8438C3.63818 14.8438 4.6875 13.7944 4.6875 12.5ZM5.31846 17.3378C4.02402 17.3378 2.97471 18.3871 2.97471 19.6815C2.97471 20.976 4.02402 22.0253 5.31846 22.0253C6.61289 22.0253 7.66221 20.976 7.66221 19.6815C7.66221 18.3872 6.61284 17.3378 5.31846 17.3378ZM19.6815 17.3378C18.3871 17.3378 17.3378 18.3871 17.3378 19.6815C17.3378 20.976 18.3871 22.0253 19.6815 22.0253C20.976 22.0253 22.0253 20.976 22.0253 19.6815C22.0253 18.3872 20.976 17.3378 19.6815 17.3378ZM5.31846 2.97471C4.02402 2.97471 2.97471 4.02402 2.97471 5.31846C2.97471 6.61289 4.02402 7.66221 5.31846 7.66221C6.61289 7.66221 7.66221 6.61289 7.66221 5.31846C7.66221 4.02402 6.61284 2.97471 5.31846 2.97471Z" fill="white" />
          </svg>

          loading
        </button>
        :
        <button type="submit" className="max-w-fit bg-[#0079FF] hover:bg-[#026ee9] cursor-pointer transition duration-500 font-bold text-white px-4 py-4 sm:px-6 rounded-[10px]">Search</button>
      }

    </form >
  )
}